    ...
    <div class="container">
        <div class="row">
            <div class="">
                <div class="">
                    <div class="">
                        <h2 class="heading">Staff Payment Details</h2>
                    </div>
                    <div class="card-body">
                        <a href="{{ url('/payroll/create') }}" class="btn btn-success btn-sm"
                    ...
                    <form action="" class="d-flex" style="margin: -15px;">
                        <div class="form-group">
                            ...
                            <input type="search" name="search" id="" class="form-control me-2"
                                placeholder="Search by Staff Name " value="{{$search}}"
                    ...
                    <div class="card" style="margin-left:50px; margin-top:20px;">
                        ...
                        <h5><b style="font-family:'Candara'; font-size:24px;">{{ $item->staff_name }}</b></h5>
                        <p class="title" style="font-family:'Candara'; font-size:20px;">{{ $item->month }} -
                            {{ $item->year }}</p>
                        <!-- <p>{{ $item->Email }}</p> -->

                        <a href="{{ url('/payroll/' . $item->id) }}" title="View payroll"><i class="fa fa-search"
                                aria-hidden="true"></i></a>
                        <form method="POST" action="{{ url('/payroll' . '/' . $item->id) }}" accept-charset="UTF-8"
                            style="display:inline">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
                            <button type="submit" onclick="return confirm(&quot;Confirm delete?&quot;)">Delete </button>
                        </form>
                        <a href="{{ url('/payroll/' . $item->id . '/edit') }}" title="Edit payroll"><i
